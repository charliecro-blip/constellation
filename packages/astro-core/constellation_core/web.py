  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    header {
      ...
      box-shadow: 0 18px 50px rgba(43, 33, 21, 0.08);
    }
    ...
    .card {
      ...
      border-radius: 24px;
      padding: 20px;
      box-shadow: 0 10px 30px rgba(43, 33, 21, 0.05);
    }

    .stack { display: grid; gap: 16px; }
    .tight { display: grid; gap: 10px; }

    .person-grid, .context-grid {
      display: grid;
    ...
    input:focus, select:focus, textarea:focus {
      border-color: #7c6b54;
      box-shadow: 0 0 0 3px rgba(124, 107, 84, 0.15);
    }
    ...
    button {
      ...
      box-shadow: 0 10px 18px rgba(33, 31, 27, 0.12);
    }
...
<main>
  <header>
    ...
    <p class="muted">Enter two birth records, choose the relationship context, and generate a draft map. This version still asks for latitude, longitude, and timezone directly; place search comes later.</p>
  </header>
  ...
  <section class="grid">
    <form id="relationship-form" class="card stack">
        ...
        <div class="context-grid">
          <label>Relationship type
            <select name="relationship_type">
              ...
              <option value="admired_figure">Admired figure</option>
      ...
      <p class="small-note">Prototype note: birth place search is not implemented yet. Use coordinates and timezone from a trusted source for now.</p>
    </form>
    ...
    <section class="card stack">
      <div class="actions" style="justify-content: space-between; gap: 12px;">
        <div>
          <h2>Report Output</h2>
          <p class="muted">Preview or raw Markdown.</p>
        </div>
        <div class="tabs" role="tablist" aria-label="Output view">
          <button class="tab active" id="preview-tab" type="button">Preview</button>
          <button class="tab" id="markdown-tab" type="button">Markdown</button>
        </div>
      </div>
...
function person(prefix) {
  ...
  return {
    name: form.elements[`${prefix}_name`].value,
    date: form.elements[`${prefix}_date`].value,
    time: timeKnown && timeValue ? timeValue : null,
    time_known: timeKnown,
    latitude: Number(form.elements[`${prefix}_latitude`].value),
    longitude: Number(form.elements[`${prefix}_longitude`].value),
    timezone: form.elements[`${prefix}_timezone`].value
  };
}

function payloadFromForm() {
  const knownThemes = form.elements.known_themes.value
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  ...
  return {
    person_a: person("a"),
    person_b: person("b"),
    house_system: form.elements.house_system.value,
    context: {
      relationship_type: form.elements.relationship_type.value,
      status: form.elements.status.value,
      user_question: form.elements.user_question.value || null,
      origin_story: form.elements.origin_story.value || null,
...
function showError(message) {
  preview.innerHTML = `<div class="error">${escapeHtml(message)}</div>`;
...
document.getElementById("sample").addEventListener("click", () => {
  setForm(sample);
  statusEl.textContent = "Sample restored.";
});

document.getElementById("copy").addEventListener("click", async () => {
  if (!currentMarkdown) {
    statusEl.textContent = "No report to copy yet.";
    return;
  }
