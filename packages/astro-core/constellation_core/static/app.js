...
function buildContext() { const relationshipType = form.elements.relationship_type.value; return { relationship_type: relationshipType, status: inferredStatus(relationshipType), user_question: form.elements.user_question.value || null, origin_story: form.elements.origin_story.value || null, known_themes: [], house_system: form.elements.house_system.value }; }
...
async function createBirthProfile(prefix) { const response = await fetch("/birth-profiles", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(person(prefix)) }); if (!response.ok) throw new Error(`Could not save birth profile`); return response.json(); }
