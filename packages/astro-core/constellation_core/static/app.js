  ...
  try {
    ...
    if (results.length) {
      ...
      statusEl.textContent = payload.provider_available
        ...
        : (payload.message || "Preset city match found.");
      return;
    }
    statusEl.textContent = payload.message || "No city results found. Try a broader city/state/country search or use manual details.";
  } catch {
    statusEl.textContent = "City search failed. You can still use presets or manual latitude/longitude/timezone.";
  }
}
...
async function createBirthProfile(prefix) { const response = await fetch("/birth-profiles", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(person(prefix)) }); if (!response.ok) throw new Error(`Could not save person ${prefix.toUpperCase()}`); return response.json(); }
async function createSavedRelationship() { const [personA, personB] = await Promise.all([createBirthProfile("a"), createBirthProfile("b")]); const response = await fetch("/saved-relationships", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(relationshipPayload(personA.id, personB.id)) }); if (!response.ok) throw new Error("Could not save relationship"); currentSavedRelationship = await response.json(); return currentSavedRelationship; }
async function generateSavedReport(relationshipId) { const response = await fetch(`/saved-relationships/${relationshipId}/report`, { method: "POST" }); const payload = await response.json(); if (!response.ok) throw new Error(payload.detail || "Could not generate saved report"); currentMarkdown = payload.markdown; markdown.textContent = currentMarkdown; preview.innerHTML = markdownToHtml(currentMarkdown); updateDownload(currentMarkdown); setTab("preview"); }
