...
from .report import ReportSynthesisPacket
...
- If a synthesis packet is provided, treat it as the source of deterministic priority order.
- Preserve the lead pattern from the synthesis packet unless the deterministic Markdown clearly says otherwise.
- Use the top ranked patterns in the synthesis packet to shape the Overview.
- Do not elevate secondary or supporting patterns over lead-eligible patterns or higher-ranked packet patterns.
- Do not introduce new aspects, placements, houses, asteroids, signs, timing, or claims not present in the deterministic report.
- Only interpret patterns that appear in the provided report or synthesis packet.
...
- Do not turn the reading into a compatibility score; do not turn the report into compatibility scoring; do not add compatibility scores or a compatibility score.
- Do not use fate, soulmate, twin flame, destiny, fated, destined, meant-to-be, or perfect-match language.
...
def build_enhancement_user_prompt(
    ...
    packet_block = (
        ...
        else "No synthesis packet supplied; preserve priorities from the deterministic Markdown."
    )
...
Use the synthesis packet's top_ranked_patterns order and lead_pattern as the deterministic priority order when it is supplied. Do not calculate new astrology.
